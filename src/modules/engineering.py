...
class Engineering(Module):
    def __init__(self, ship, common_data=None):
        ...
        if "engineering" not in self.common_data:
            self.common_data["engineering"] = {
                ...
                "max_energy": 2000,  # Set to 2000 per feedback
                ...
                "warp_duration": 0  # Added to track Propulsion Strain
            }
    ...
    def update(self, sim_time=None):
        ...
        if eng["energy"] < eng["max_energy"]:
            eng["energy"] = min(eng["max_energy"], eng["energy"] + 5)

        # Regenerate shields (5%/second at 1 tick/second)
        if eng["shields"] < 100:
        ...
        if eng["warp_factor"] > 0:
            warp_cost = min(1000, eng["warp_factor"] * 90)  # Warp 9 = 810 units/second
            if eng["energy"] >= warp_cost:
                eng["energy"] -= warp_cost
                eng["warp_duration"] += 1  # Increment for Propulsion Strain
            else:
                # Fallback to half cost if energy tight
                warp_cost = eng["warp_factor"] * 45  # Warp 9 = 405 units/second
                if eng["energy"] >= warp_cost:
                    eng["energy"] -= warp_cost
                    eng["warp_duration"] += 1
                else:
                    eng["warp_factor"] = 0  # Warp disengages
                    eng["warp_duration"] = 0
                    self.common_data["debug"].append({"timestamp": sim_time,
                                                     "message": "ENG: Warp disengaged - insufficient energy"})
        else:
            eng["warp_duration"] = 0  # Reset when not in warp
            eng["impulse_speed"] = min(100, prop_alloc * 2)  # 1% alloc = 2% speed
        ...
        # Power surge event (5% chance at >95% energy)
        if eng["energy"] > eng["max_energy"] * 0.95 and random.random() < 0.05:
            ...
            eng["surge"] = False  # Reset surge unless triggered

        # Shield overload event (10% chance if alloc > 80 and health < 20)
        ...
        # Propulsion strain event (5% chance if warp > 7 for >10 seconds)
        ...
        if cmd == "status":
            ...
            try:
                ...
                if 0 <= factor <= 10:
                    ...
                    warp_cost = min(1000, factor * 90)  # Updated to match 810 at Warp 9
                    self.common_data["debug"].append({"timestamp": sim_time,
